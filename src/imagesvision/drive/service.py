...
class DriveService:
    ...
    def upload_file(self, name, filePath, folder_id, desc):
    ...
    def writer_permission(self, email, file_id):
        batch = self.drive_service.new_batch_http_request(callback=self.__callback)
    ...
    def transfer_ownership(self, email, file_id):
        batch = self.drive_service.new_batch_http_request(callback=self.__callback)
        permission = {"type": "user", "role": "owner", "emailAddress": email}
        batch.add(
            self.drive_service.permissions().create(
                transferOwnership=True, fileId=file_id, body=permission, fields="id",
            )
        )
        batch.execute()
    ...
    def list_folders_by_name(self, name):
        return self.__list_items_by_name(
    ...
    def __list_items_by_name(self, name, extraQuery=None):
        ...
        while True:
            ...
            for file in response.get("files", []):
                # Process change
                itemsList.append({"id": file.get("id")})
            page_token = response.get("nextPageToken", None)
            if page_token is None:
                break
